...
void insertEntry(dataEntry new_entry){
	...
	new_entry_bucket = recuperarBucket(nbucket,0);
	...
	while ((i<28)&&(!found)) {
		if (new_entry_bucket->freeSpace[i] == 0){
			...
			escreverBucket(nbucket, new_entry_bucket,0);
			found = 1;	
		}
		i++;
	}
	...
	while(!consegui) {
		if(new_entry_bucket->overflow == 0){	//se não existir bucket overflow
			...
			escreverBucket(header[6],new_overflow,1);
			consegui = 1;
		}
		else{		//se existir bucket overflow
			nOverflow = new_entry_bucket->overflow;
			new_entry_bucket = recuperarBucket(new_entry_bucket->overflow,1);
			i=0;
			while ((i<28)&&(!found)) {
				if (new_entry_bucket->freeSpace[i] == 0){
					new_entry_bucket->entries[i].key = new_entry.key;
					new_entry_bucket->entries[i].rid = new_entry.rid;
					new_entry_bucket->freeSpace[i] = 1;
					escreverBucket(nOverflow, new_entry_bucket,1);
					found = 1;	
					consegui = 1;
				}
				i++;
